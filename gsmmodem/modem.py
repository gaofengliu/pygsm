...
class GsmModem(SerialComms):
    ...
    CMTI_REGEX = re.compile(r'^\+CMTI:\s*"([^"]+)",\s(\d+)$')    # modified by lgf
    ...
    def _handleSmsReceived(self, notificationLine):
        ...
        self.log.debug(notificationLine)
        if cmtiMatch:
            self.log.debug('cmtiMatch = ture')
    ...
    def readStoredSms(self, index, memory=None):
        ...
        if self._smsTextMode:
            ...
            if not cmgrMatch:
                raise CommandError('Failed to parse PDU-mode SMS message +CMGR response: {0}'.format(msgData[0]))
